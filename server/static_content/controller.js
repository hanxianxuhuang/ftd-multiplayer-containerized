...
var game_status = 0; // 0 if game is on, 2 if game is over
...
function update(message){
	if (message["type"] == "data"){
		for (let new_square in message["new_squares"]){
			stage.squares[new_square] = message["new_squares"][new_square];
		}
		for (let updated_square in message["updated_squares"]){
			if (updated_square in stage.squares){
				stage.squares[updated_square].x = message["updated_squares"][updated_square].x;
				stage.squares[updated_square].y = message["updated_squares"][updated_square].y;
				stage.squares[updated_square].health = message["updated_squares"][updated_square].health;
			}
		}
		for (let deleted_square of message["deleted_squares"]){
			if (deleted_square in stage.squares){
				delete stage.squares[deleted_square];
			}
		}
		...
		stage.id = message["id"];
		stage.square_id = message["square_id"];
	}
}
...
function moveByKey(event){
	...
	if (game_status == 0 && socket != null && stage != null){
		...
		if(key in moveMap){
			...
			socket.send(JSON.stringify({"type" : "move", "id" : stage.id, "direction" : moveMap[key]}));
		}

		// reload bullets and restore health
		if (key == 'e' && game_status == 0){
			// let the game model know the user wants to retreat
			socket.send(JSON.stringify({"type" : "retreat", "id" : stage.id}));
		}
	}
}
...
function clickMouse(event){
	if (game_status == 0 && socket != null && stage != null){
		...
		//         audio.play();
		// }
		// let the game model know the user has clicks the mouse (so the player can fire)
		socket.send(JSON.stringify({"type" : "fire", "id" : stage.id, "x" : stage.mouseX, "y" : stage.mouseY}));
	}
}
...
function switch_page(page){
	...
	if (page == "play"){
		...
		// show the current game status (started or ended)
		if (game_status == 0){
			$("#ui_notification").append("<p>Game Started</p>");
